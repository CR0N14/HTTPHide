...
def is_steg_response(response: Response):
    return
...
def main():
    ...
    while (True):
        ...
        3. repeat
        '''
        response = send_stego_data_to_listener(command_line_output)
        if is_steg_response(response):
            command_line_input = get_command_line_input(response)
